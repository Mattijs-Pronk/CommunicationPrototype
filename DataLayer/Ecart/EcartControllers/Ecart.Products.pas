...
begin
  ...
  try
    HttpClient.CustomHeaders['Authorization'] := Local_AccesToken;
    HttpClient.CustomHeaders['Content-Type'] := 'application/json';
    ResponseContent := HttpClient.Get(Ecart_BaseURL + 'products/' + UIntToStr(productId) + '/images').ContentAsString;
...
begin
  ...
  try
    HttpClient.CustomHeaders['Authorization'] := Local_AccesToken;
    HttpClient.CustomHeaders['Content-Type'] := 'application/json';
    ResponseContent := HttpClient.Get(Ecart_BaseURL + 'products/' + UIntToStr(productId) + '/images/' + UIntToStr(imageId)).ContentAsString;
...
function ProductsController.PostImage(productId: UInt64; NewImageJSON: TJSONObject): TJSONValue;
var
  ResponseContent: string;
  RequestBody: TStringStream;
begin
  HttpClient := TNetHTTPClient.Create(nil);
  RequestBody := TStringStream.Create(NewImageJSON.ToJSON, TEncoding.UTF8);
  try
    HttpClient.CustomHeaders['Authorization'] := Local_AccesToken;
    HttpClient.CustomHeaders['Content-Type'] := 'application/json';
    ResponseContent := HttpClient.Put(Ecart_BaseURL + 'products/' + UIntToStr(productId) + '/images', RequestBody).ContentAsString;

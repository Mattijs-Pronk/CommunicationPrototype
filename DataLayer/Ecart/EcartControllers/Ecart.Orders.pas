...
type
  ...
  public
    ...
    function PostOrderRisk(orderId: UInt64; NewOrderRisk: TJSONObject): TJSONValue;
...
function OrdersController.PostOrderRisk(orderId: UInt64; NewOrderRisk: TJSONObject): TJSONValue;
var
  ResponseContent: string;
  RequestBody: TStringStream;
begin
...
function OrdersController.DeleteOrderRisk(orderId: UInt64; riskId: UInt64): TJSONValue;
var
  ResponseContent: string;
begin
  HttpClient := TNetHTTPClient.Create(nil);
  try
    HttpClient.CustomHeaders['Authorization'] := Local_AccesToken;
    HttpClient.CustomHeaders['Content-Type'] := 'application/json';
    ResponseContent := HttpClient.Delete(Ecart_BaseURL + 'orders/' + UIntToStr(orderId) + 'risks' + UIntToStr(riskId)).ContentAsString;
